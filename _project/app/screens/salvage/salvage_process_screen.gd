...
const SalvageComponentTokenScene := preload("res://_project/app/screens/salvage/salvage_component_token.tscn")
...
var _run_summary_popup: RunSummaryPopup = null
...
func _show_results_popup() -> void:
	...
	_run_summary_popup = RunSummaryPopupScene.instantiate() as RunSummaryPopup
	if _run_summary_popup == null:
		station_requested.emit()
		return

	var run_stats: Dictionary = _run_result.to_stats_dict(_salvage_item_total_count) if _run_result != null else {}
...
func _create_active_item_button(item_data: SalvageItemData, quantity: int = 1) -> TextureButton:
	...
	if button.texture_normal == null:
		button.texture_normal = Utils.create_placeholder_texture(
			item_data.get_rarity_color() if item_data else Color.WHITE, active_item_display_size)
