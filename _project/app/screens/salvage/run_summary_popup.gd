...
func _apply_stats() -> void:
	...
	_time_value.text = Utils.format_elapsed_time(float(_run_stats.get("time_elapsed", 0.0)))
...
func _build_entry_row(entry: Dictionary) -> HBoxContainer:
	...
	if icon_rect.texture == null:
		icon_rect.texture = Utils.create_placeholder_texture(
			item_data.get_rarity_color() if item_data else Color.WHITE, ENTRY_ICON_SIZE)
...
func _get_end_summary_text() -> String:
	...
	return _run_result.get_end_reason_text()
