...
## Applies the label font to every Label/Button/RichTextLabel under root (inclusive).
func apply_label_fonts(root: Node) -> void:
	if root is Label or root is Button:
		apply_label_font(root as Control)
	elif root is RichTextLabel:
		(root as RichTextLabel).add_theme_font_override("normal_font", LABEL_FONT)
	for child in root.get_children():
		apply_label_fonts(child)
