...
## Physical heft class. Each class maps to a predetermined internal weight and
## physics profile (see WEIGHT_CLASS_PROFILES).
...
## Internal weight float, derived from the weight class profile. Read-only:
## authored via weight_class, consumed by the pull/drop physics.
var weight: float:
	get:
