...
# Piles are generic and uncolored; loot is determined by the shared pile_data,
# not by any per-pile rarity.
...
func _ready() -> void:
	_pile_textures = [
		preload("res://_project/salvage/pile/sprites/trash_pile_1.png"),
		preload("res://_project/salvage/pile/sprites/trash_pile_2.png"),
		preload("res://_project/salvage/pile/sprites/trash_pile_3.png"),
	]
	deactivate()
...
func activate(spawn_position: Vector2, level: Node, target_height: float, _new_rotation: float) -> void:
	...
	if sprite and _pile_textures.size() > 0:
		...
		# Piles are uncolored: reset the shader tint to neutral white.
		if sprite.material:
...
# Surface line (magnet pull Phase 2 detection)
...
func _generate_surface_line() -> void:
	...
	if _surface_line:
		_surface_line.visible = false  # Detection geometry only, never rendered
	else:
