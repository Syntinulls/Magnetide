...
## then a sharp falloff to a drift. The visual grows as the flame decelerates,
## shifts color along color_ramp over its lifetime, and spins at a per-flame
...
@export var falloff_duration: float = 0.75
## Drift speed the flame settles at after the falloff.
@export var min_speed: float = 150.0
## Sprite scale at full speed, so flames are visible right out of the muzzle.
@export var min_visual_scale: float = 0.7
## Sprite scale once the flame has fully decelerated.
@export var max_visual_scale: float = 1.5
## Response curve applied to the speed-driven growth: scale follows the same
## pattern as the speed falloff, but an exponent below 1 makes it ramp ahead of
## the speed change (more volatile early growth). 1.0 = linear with speed.
@export_range(0.1, 1.0, 0.05) var scale_response_exponent: float = 0.5
...
func tick(projectile: Projectile, elapsed: float, delta: float) -> void:
	...
	if visual == null:
		return
	# The flame grows as it slows, but outpaces the deceleration: the speed-driven
	# progress is pushed through a sub-linear response curve so scale surges early
	# while still tracking the shape of the speed falloff.
	var speed_ratio := _current_speed(projectile, elapsed) / maxf(projectile.speed, 0.01)
	var growth := pow(clampf(1.0 - speed_ratio, 0.0, 1.0), maxf(scale_response_exponent, 0.01))
